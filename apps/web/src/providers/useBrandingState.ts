...
export interface KafilBrandingContextValue {
  isAdmin: boolean;
...
export function useBrandingState(
  ...
  const resetToFactory = useCallback(async () => {
    if (!isAdmin) throw new Error("Branding is read-only for this session");
    const orphansBefore = state.orphanCandidates;
    await resetBrandingApi({ expectedRevision: state.committed.revision });
