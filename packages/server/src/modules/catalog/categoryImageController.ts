...
function resolveCategoryImagePath(rawFileName: string) {
  const fileName = decodeURIComponent(rawFileName);
  if (!/^[0-9a-f-]{36}\.(?:avif|gif|jpg|png|webp)$/i.test(fileName)) {
    HttpError.create(400, "Invalid category image file name");
  }
...
export class CategoryImageController {
  ...
  async upload(
    ...
    const { directory, fileName } = resolveCategoryImagePath(rawFileName);
    const expectedType = CATEGORY_IMAGE_TYPES[
      extname(fileName).toLowerCase() as keyof typeof CATEGORY_IMAGE_TYPES
    ];
    if (!contentType || contentType.toLowerCase() !== expectedType) {
      HttpError.create(415, "Unsupported category image type");
    }
    if (body.byteLength === 0 || body.byteLength > MAX_CATEGORY_IMAGE_SIZE) {
      HttpError.create(413, "Category image must be between 1 byte and 5 MB");
    }
    ...
    return {
      path: `${CATEGORY_IMAGE_SERVE_PREFIX}${encodeURIComponent(fileName)}`,
    };
  }
    ...
    try {
      ...
      return new Response(new Uint8Array(bytes), {
        headers: {
          ...
          "content-type": CATEGORY_IMAGE_TYPES[
            extname(fileName).toLowerCase() as keyof typeof CATEGORY_IMAGE_TYPES
          ],
        },
      });
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        HttpError.notFound("Category image not found");
      }
      throw error;
    }
  }
